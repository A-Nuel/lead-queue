...
create table if not exists leads (
  ...
  line_type text default 'unknown',         -- 'mobile' | 'landline' | 'unknown'
  whatsapp_link text,                       -- wa.me/<phone_e164 digits only>, null if landline
...
-- If you already ran an earlier version of this schema (leads table exists
-- without line_type), run this migration once instead of the create table above:
-- alter table leads add column if not exists line_type text default 'unknown';
